...
import { getTemplateById, mockTasks, getEligibilityCriteriaById } from '../../../data';
import type { TaskGroup, TaskType, EligibilityCriteria, Task } from '../../../types';
...
export function TemplateDetailPage() {
    ...
    // Template state (mutable copy)
    const [taskGroups, setTaskGroups] = useState<TaskGroup[]>(
        () => initialTemplate?.taskGroups.map(g => ({ ...g, tasks: [...g.tasks] })) || []
    );
    ...
    const [expandedGroups, setExpandedGroups] = useState<Set<string>>(
        new Set(initialTemplate?.taskGroups.map(g => g.id) || [])
    );
    ...
    // Task Edit Modal state - now uses type-specific modals
    const [showTaskEditModal, setShowTaskEditModal] = useState(false);
    const [editingTask, setEditingTask] = useState<Task | null>(null);

    // Eligibility criteria state
    const [showEligibilityModal, setShowEligibilityModal] = useState(false);
    const [eligibilityContext, setEligibilityContext] = useState<'TEMPLATE' | 'TASK_GROUP' | 'TASK'>('TEMPLATE');
    ...
    const [templateEligibility, setTemplateEligibility] = useState<EligibilityCriteria | undefined>(
        initialTemplate?.eligibilityCriteriaId ? getEligibilityCriteriaById(initialTemplate.eligibilityCriteriaId) : undefined
    );
    ...
    const [groupEligibility, setGroupEligibility] = useState<Record<string, EligibilityCriteria | undefined>>(() => {
        ...
        initialTemplate?.taskGroups.forEach(g => {
        ...
        return (
            <div className="page-enter">
                <div className="text-center py-12">
                    ...
                    <p className="text-secondary mb-4">The template you're looking for doesn't exist.</p>
    ...
    // Open eligibility modal for template
    const openTemplateEligibility = () => {
        setEligibilityContext('TEMPLATE');
        setEligibilityEntityId(null);
        setEligibilityEntityName(initialTemplate.name);
    ...
    const handleEligibilitySave = (criteria: EligibilityCriteria) => {
        if (eligibilityContext === 'TEMPLATE') {
            setTemplateEligibility(criteria);
        } else if (eligibilityContext === 'TASK_GROUP' && eligibilityEntityId) {
    ...
    const handleEligibilityRemove = () => {
        ...
        // Perform the removal
        if (context === 'TEMPLATE') {
            setTemplateEligibility(undefined);
        } else if (context === 'TASK_GROUP' && entityId) {
        ...
        showToast({
            ...
            undoAction: () => {
                ...
                if (previousCriteria) {
                    if (context === 'TEMPLATE') {
                        setTemplateEligibility(previousCriteria);
                    } else if (context === 'TASK_GROUP' && entityId) {
    ...
    return (
        <div className="page-enter">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-3 mb-1">
                                <h1 className="page-title">{initialTemplate.name}</h1>
                                <Badge variant={initialTemplate.isActive ? 'success' : 'secondary'}>
                                    {initialTemplate.isActive ? 'Active' : 'Draft'}
                                </Badge>
                                <Badge variant="outline">v{initialTemplate.version}</Badge>
                            </div>
                            <p className="page-description">{initialTemplate.description}</p>
                        </div>
                    </div>
                    <div className="page-actions">
                        <EligibilityCriteriaBadge
                            criteria={templateEligibility}
                            onClick={openTemplateEligibility}
                        />
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
                ...
                <Card>
                    <CardBody className="p-4">
                        <div className="text-2xl font-bold">{initialTemplate.clientName || 'All'}</div>
                        <div className="text-sm text-secondary">Client</div>
                    </CardBody>
                </Card>
                <Card>
                    <CardBody className="p-4">
                        <div className="text-2xl font-bold">{initialTemplate.createdBy}</div>
